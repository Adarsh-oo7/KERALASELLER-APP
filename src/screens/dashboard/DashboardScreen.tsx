...
//         const store = raw?.store_profile || raw?.data || raw || {};
//         const seller = raw?.seller || store?.seller || {};

//         if (Object.keys(store).length > 0) {
//           setStoreData({
//             ...store,
//             seller,
//             seller_phone:   seller.phone   || store.seller_phone   || '',
//             seller_address: seller.address || store.seller_address || store.address || '',
//           });
//         }
//       } else if (storeRes.status === 'rejected' && storeRes.reason?.message?.includes('timeout')) {
...
//           (s: number, o: any) => o.status === 'DELIVERED' ? s + (parseFloat(o.total_amount) || 0) : s,
//           0,
//         );
...
//   // ── FIX: Logout — clears auth AND resets navigation to Login ──────────────
...
//             } catch (e) {
//               // Even if logout API call fails, clear local state and navigate
//               console.warn('Logout API error (ignored):', e);
//             } finally {
//               // FIX: Always reset navigation — this is what was missing
//               navigation.reset({
//                 index: 0,
//                 routes: [{ name: 'Login' }], // ← change 'Login' to match your auth screen name
//               });
//             }
//           },
//         },
...
//   // ── FIX: Memoize shopUrl — was being recomputed 3× per render ─────────────
//   const shopUrl = useMemo(() => {
//     if (!storeData?.name) return 'https://www.keralasellers.in/shop';
...
//     const shopSlug     = slug(storeData.name);
//     const locationPart = slug(storeData.seller_address || storeData.address || '').split('-')[0] || '';
//     const combined     = locationPart ? `${shopSlug}-${locationPart}` : shopSlug;
//     const finalSlug    = combined.length >= 3
//       ? combined
//       : `shop-${storeData.seller_phone || storeData.phone || 'store'}`;

//     const phone = storeData.seller_phone
//       || storeData.seller?.phone
//       || storeData.phone
//       || storeData.whatsapp_number
//       || '';

//     return `https://www.keralasellers.in/shop/${finalSlug}?id=${phone}`;
//   }, [storeData]);

//   // ── FIX: Stable callbacks using memoized shopUrl ──────────────────────────
//   const shareStore = useCallback(async () => {
//     try {
//       await Share.share({ message: `Check out my Kerala Sellers store: ${shopUrl}`, url: shopUrl });
//     } catch {
...
//     Linking.openURL(shopUrl).catch(() => Alert.alert('Error', 'Could not open link'));
//   }, [shopUrl]);

//   const navigateTo = useCallback((screen: string | null, label: string) => {
//     if (screen) navigation.navigate(screen);
//     else Alert.alert('Coming Soon!', `${label.replace('\n', ' ')} will be available soon.`);
//   }, [navigation]);
...
//   ], [totalRevenue, totalOrders, totalProducts, newOrders]);
...
//         <View style={s.setupIconWrap}><Text style={{ fontSize: 40 }}>🏪</Text></View>
...
//             : <View style={s.storeLogoPlaceholder}><Ionicons name="storefront" size={24} color="#3b82f6" /></View>
//           }
//           <View style={{ flex: 1 }}>
//             <Text style={s.storeName} numberOfLines={1}>{storeData?.name}</Text>
//             <Text style={s.storeTagline} numberOfLines={1}>{storeData?.tagline || 'Kerala Sellers'}</Text>
//           </View>
//         </View>
...
//           <View style={s.subIconWrap}><Ionicons name="lock-closed-outline" size={20} color="#d97706" /></View>
//           <View style={{ flex: 1 }}>
...
//               style={s.actionCard}
//               activeOpacity={0.75}
//               onPress={() => navigateTo(a.screen, a.label)}
//             >
...
//             <Text style={s.urlText} numberOfLines={1} ellipsizeMode="middle">{shopUrl}</Text>
//           </View>
...
//                 <Text style={s.topMedal}>{['🥇', '🥈', '🥉'][idx]}</Text>
...
const DashboardScreen: React.FC<DashboardScreenProps> = ({ navigation }) => {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      if (ordersRes.status === 'fulfilled' && productsRes.status === 'fulfilled') {
        ...
        const totalRevenue = orders.reduce(
          (sum: number, o: any) => o.status === 'DELIVERED' ? sum + (parseFloat(o.total_amount) || 0) : sum,
          0,
        );
        const topProducts = [...products]
          .sort((a: any, b: any) => (b.sold_count || 0) - (a.sold_count || 0))
  ...
  // ── FIX 1: Logout via event — navigation.reset() targeted tab stack, not root ──
  const handleLogout = useCallback(() => {
    Alert.alert(
      'Logout',
      'Are you sure you want to logout?',
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              // Fires to AppNavigator → setIsAuthenticated(false) → Login screen instantly
  ...
  }, []); // no navigation dependency — event handles routing

  // ── FIX 2: shopUrl — use API slug directly, not regenerated from store name ──
  // Logs showed: slug: "demk", but old code was building slug from storeData.name
  const shopUrl = useMemo(() => {
    if (!storeData) return 'https://www.keralasellers.in/shop';

    const slug  = storeData.slug
      || storeData.name?.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '')
      || 'store';
    ...
    return phone
      ? `https://www.keralasellers.in/shop/${slug}?id=${phone}`
      : `https://www.keralasellers.in/shop/${slug}`;
  }, [storeData]);
  ...
  const STAT_CARDS = useMemo(() => [
    {
      label: 'Revenue',
      value: `₹${totalRevenue >= 1000 ? (totalRevenue / 1000).toFixed(1) + 'k' : totalRevenue.toLocaleString('en-IN')}`,
  ...
  if ((timeoutError || error) && !hasStore) return (
    <View style={s.centered}>
      <View style={s.errorIconWrap}>
        <Ionicons
          name={timeoutError ? 'time-outline' : 'cloud-offline-outline'}
          size={36}
          color="#dc2626"
        />
      </View>
    ...
    >
      <View style={s.setupCard}>
        ...
        <Text style={s.setupDesc}>
          Complete your store profile to start selling to customers across Kerala.
        </Text>
        <TouchableOpacity style={s.setupBtn} onPress={() => navigation.navigate('CreateShop')}>
          <Ionicons name="storefront-outline" size={18} color="white" />
          <Text style={s.setupBtnText}>Setup Store Now</Text>
    ...
    >
      ...
      <View style={s.storeHeader}>
        <View style={s.storeHeaderLeft}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.storeName} numberOfLines={1}>{storeData?.name}</Text>
            <Text style={s.storeTagline} numberOfLines={1}>
              {storeData?.tagline || 'Kerala Sellers'}
            </Text>
          </View>
        </View>
        <TouchableOpacity
      ...
      {hasActiveSub ? (
        <View style={[s.subCard, expiringSoon && s.subCardWarn]}>
          <View style={[s.subIconWrap, { backgroundColor: expiringSoon ? '#fef3c7' : '#eff6ff' }]}>
            <Ionicons
              name={expiringSoon ? 'warning-outline' : 'star'}
              size={20}
              color={expiringSoon ? '#d97706' : '#3b82f6'}
            />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={s.subTitle}>{subscriptionInfo?.plan_name || 'Premium'} Plan</Text>
            <Text style={s.subDetails}>
              ...
              {' · '}
              {subscriptionInfo?.product_limit || '∞'} products
            </Text>
            {expiringSoon && (
              <Text style={s.subWarnText}>Renew soon to avoid interruption</Text>
            )}
          </View>
          <TouchableOpacity
            style={[s.subBtn, expiringSoon && s.subBtnWarn]}
        ...
        {topProducts.length > 0 ? (
          <View style={s.topProductsCard}>
            {topProducts.map((item: any, idx: number) => (
              <View
                key={item.id}
                style={[s.topRow, idx < topProducts.length - 1 && s.topRowBorder]}
              >
                <Text style={s.topMedal}>{['🥇', '🥈', '🥉'][idx]}</Text>
                <Text style={s.topName} numberOfLines={1}>{item.name}</Text>
                <View style={s.topSoldWrap}>
          ...
          <View style={s.emptyCard}>
            ...
            <Text style={s.emptyDesc}>
              Add products and share your store link to start selling
            </Text>
          </View>
        )}
      </View>
...
const s = StyleSheet.create({
  screen:        { flex: 1, backgroundColor: '#f1f5f9' },
  // FIX 3: Was 24 — floating tab bar is ~102px tall, content was hidden behind it
  scrollContent: { paddingBottom: 110 },
  // FIX 3 (same): setup screen also needs bottom clearance
  setupScroll:   { flexGrow: 1, justifyContent: 'center', padding: H_PAD, paddingBottom: 110 },
